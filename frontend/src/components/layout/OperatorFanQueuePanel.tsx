...
const FAN_LTV: Record<string, string> = {
  'fan-mike': '$270', 'fan-jane': '$1.2k', 'fan-sam': '$3.4k', 'fan-dave': '$95', 'fan-new88': '$45',
}

export function OperatorFanQueuePanel({ fans, selectedFanId, onSelectFan, livePendingCounts }: OperatorFanQueuePanelProps) {
  const totalPending = livePendingCounts
    ? Object.values(livePendingCounts).reduce((sum, c) => sum + c, 0)
    : fans.reduce((sum, f) => sum + f.pendingCount, 0)
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-1 mb-0.5">
                    ...
                    {(livePendingCounts?.[fan.id] ?? fan.pendingCount) > 0 && (
                      <span
                        style={{
                      ...
                      >
                        {livePendingCounts?.[fan.id] ?? fan.pendingCount}
                      </span>
                    )}
                  </div>
                  <div
                  ...
                  >
                    {FAN_LTV[fan.id] ?? '$1.2k'} · {tier.label}
                  </div>
                </div>
              </div>
            </motion.div>
