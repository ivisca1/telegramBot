...
export function ChatArea({ messages, isLoading, onSend, onAbort, onApprove, onReject, onEdit, demoMode, className }: ChatAreaProps) {
                        ...
                        {message.status === 'pending' && editingId !== message.id && (
                          <>
                            {message.status === 'pending' && (
                              <div className="flex items-center gap-1.5 mt-2 text-[10px] text-purple-400 bg-purple-900/20 px-2 py-1 rounded-full w-fit">
                                <span>💰</span>
                                <span>~78% chance of $35+ purchase</span>
                              </div>
                            )}
