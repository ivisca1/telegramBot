...
const AGENT_DISPLAY: Record<string, { label: string; desc: string; dot: string }> = {
  'of-chatter':       { label: 'OF Chatter',        desc: 'Warm, flirty, feminine',       dot: 'bg-pink-400' },
  'of-chatter-shy':   { label: 'Shy & Mysterious',  desc: 'Hesitant, slow-burn intimacy', dot: 'bg-violet-400' },
  'of-chatter-slutty':{ label: 'Aggressive',         desc: 'Dominant, direct, intense',    dot: 'bg-orange-400' },
}

export default function AgencyPulseBar({
  selectedAgent = 'of-chatter',
  availableAgents = ['of-chatter'],
  onAgentChange,
}: AgencyPulseBarProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-5">
        <StatItem label="Today" value="$4,230" valueStyle={{ color: 'hsl(38 72% 62%)' }} />
        <StatItem label="msgs" value="847" />
        <StatItem label="PPV" value="12.4%" valueStyle={{ color: 'hsl(158 50% 58%)' }} />
        <StatItem label="overrides" value="3" valueStyle={{ color: 'hsl(36 90% 60%)' }} />
      </div>
